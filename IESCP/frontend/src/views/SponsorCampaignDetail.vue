<template>
    <div>
      ...
      <div class="container text-center">
        ...
        <p><strong>Clicks : </strong> {{ campaignDetail.clicks }}</p>
        <p><strong>Impressions : </strong> {{ campaignDetail.impressions }}</p>
        <p><strong>Created At : </strong> {{ campaignDetail.created_at }}</p>
        <router-link :to="`/new-adrequest/${campaignDetail.id}`" class="btn btn-primary">+ AdRequest</router-link>
        </div>
        <div v-else>
          <p>Loading data...</p>
        </div>
