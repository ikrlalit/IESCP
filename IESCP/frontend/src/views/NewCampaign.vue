    ...
    <div>
        ...
        <div class="container mt-4 py-5">
            <div class="card bg-dark text-white p-5" style="width: 40rem; margin-left: auto; margin-right: auto;">
                <div class="card-body">
                    <h3 class="card-title pb-3" style="text-align: center;">New Campaign</h3>
                    ...
                    <button class="btn btn-primary" @click="submitCampaign">Create</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        async submitCampaign() {
            try {
                const apiURL = 'http://127.0.0.1:5000/api/NewCampaign';
                const token = localStorage.getItem('user-token');
                const response = await axios.post(apiURL, this.campaign, {
                    headers: {
                ...
                if (response.data.status_code === 200) {
                    ...
                    this.errorMessage = response.data.message || 'An error occurred while creating the campaign.';
                    this.successMessage = '';
                }
            } catch (error) {
                this.errorMessage = error.response?.data?.message || 'An error occurred while creating the campaign.';
                this.successMessage = '';
            }
        }
    }
